import os
import sys
from pathlib import Path

import numpy as np
import torch
from scipy.ndimage.morphology import binary_dilation

import utils
from config import Config
from dataloaders.loader import get_transform
from dataloaders.reporters import GroundTruthProcessor, SequenceScoreTracker
from dataloaders.tbd_loader import (
    get_falling_dataset,
    get_tbd3d_dataset,
    get_tbd_dataset,
    get_wildfmo_dataset,
    get_youtube_dataset,
)
from helpers.torch_helpers import renders2traj, renders2traj_bbox


def get_figure_images(
    dataset_folder: Path,
    config: Config,
    encoder: torch.nn.Module,
    rendering: torch.nn.Module,
    device: torch.device,
    mode: str,
    seqi: int,
    frmi: int,
    results_mode: bool = False,
    n_occ: int = 2,
) -> None:
    log_folder = "./output/"
    if not os.path.exists(log_folder):
        os.makedirs(log_folder)

    tbd_folder = str(dataset_folder / "TbD_GC/")
    tbd3d_folder = str(dataset_folder / "TbD-3D/")
    falling_folder = str(dataset_folder / "falling_objects/")
    wildfmo_folder = str(dataset_folder / "wildfmo/")
    youtube_folder = str(dataset_folder / "youtube/")
    train_folder = str(dataset_folder / "ShapeNetv2/ShapeBlur1000STL.hdf5")
    validation_folder = str(dataset_folder / "ShapeNetv2/ShapeBlur20STL.hdf5")

    medn = 7
    gamma_c = 1  # 0.7
    ext_factor = 4
    multi_f = 1

    do_deblatting = False
    do_sota18 = False
    do_deblurgan = False

    do_tbdo = False

    if do_sota18:
        from helpers.sota18_runner import run_sota18
    if do_deblurgan:
        sys.path.insert(0, "./DeblurGANv2")
        from helpers.deblurgan_runner import run_deblurgan

    if mode == "tbd":
        files = get_tbd_dataset(tbd_folder)
        folder = tbd_folder
    elif mode == "tbd3d":
        files = get_tbd3d_dataset(tbd3d_folder)
        folder = tbd3d_folder
        medn = 50
    elif mode == "tbdfalling":
        files = get_falling_dataset(falling_folder)
        folder = falling_folder
        medn = 50
        ext_factor = 4
    elif mode == "wildfmo":
        files = get_wildfmo_dataset(wildfmo_folder)
        folder = wildfmo_folder
        medn = 50
        ext_factor = 1
    elif mode == "train":
        files = config.render_objs_train
        folder = train_folder
        ext_factor = 1
    elif mode == "val":
        files = config.render_objs_train
        folder = validation_folder
        ext_factor = 1
    elif mode == "youtube":
        files = get_youtube_dataset(youtube_folder)
        folder = youtube_folder
        ext_factor = 4
        medn = 9
    else:
        print("Mode not found!")

    for kkf, ff in enumerate(files):
        isdigit = str(seqi).isdigit()
        if (isdigit and kkf != seqi) or (not isdigit and seqi not in ff):
            continue

        gtp = GroundTruthProcessor(ff, kkf, folder, medn, 0)
        do_tbdo = do_tbdo and gtp.w_trajgt

        seq_score_tracker = SequenceScoreTracker(gtp.nfrms)
        seq_score_tracker_tbd = SequenceScoreTracker(gtp.nfrms, "tbd", False)
        seq_score_tracker_tbd3d = SequenceScoreTracker(
            gtp.nfrms, "tbd3d", False
        )

        est_traj = None
        for kk in range(gtp.nfrms):
            I, B = gtp.get_img(kk)
            if kk != frmi:
                continue

            gt_traj, radius, bbox = gtp.get_trajgt(kk)
            gt_hs = gtp.get_hs(kk)

            if gtp.use_hs:
                gt_hs[gt_hs < 0] = 0

            if not gtp.w_trajgt:
                if gtp.use_hs:
                    bbox, radius = utils.fmo_detect_hs(gt_hs, B)
                else:
                    bbox, radius = utils.fmo_detect_maxarea(I, B)
            else:
                bbox = utils.extend_bbox_uniform(bbox, radius, I.shape)

            bbox_tight = utils.extend_bbox_uniform(bbox.copy(), 10, I.shape)
            if gtp.use_hs and not gtp.syn_mode:
                bbox_tight = utils.bbox_fmo(bbox_tight, gt_hs, B)

            if n_occ > 9:
                bbox_tight = utils.extend_bbox_uniform(
                    bbox_tight.copy(), 3, I.shape
                )
                ext2 = (
                    (bbox_tight[2] - bbox_tight[0])
                    - (bbox_tight[3] - bbox_tight[1])
                ) // 2 + (bbox_tight[2] - bbox_tight[0]) // 4
                if ext2 > 0:
                    bbox_tight = utils.extend_bbox_nonuniform(
                        bbox_tight.copy(), [0, ext2], I.shape
                    )
            else:
                bbox_tight = utils.extend_bbox_uniform(
                    bbox_tight.copy(), 3, I.shape
                )

            bbox = utils.extend_bbox(
                bbox_tight.copy(),
                ext_factor * np.max(radius),
                config.resolution_y / config.resolution_x,
                I.shape,
            )

            im_crop = utils.crop_resize(
                I, bbox, (config.resolution_x, config.resolution_y)
            )
            bgr_crop = utils.crop_resize(
                B, bbox, (config.resolution_x, config.resolution_y)
            )

            eps = 0
            preprocess = get_transform()
            input_batch = (
                torch.cat((preprocess(im_crop), preprocess(bgr_crop)), 0)
                .to(device)
                .unsqueeze(0)
                .float()
            )
            with torch.no_grad():
                latent = encoder(input_batch)
                steps = gtp.nsplits * multi_f
                times = torch.linspace(0 + eps, 1 - eps, steps).to(device)
                renders = rendering(latent, times[None])

            renders_rgba = (
                renders[0].data.cpu().detach().numpy().transpose(2, 3, 1, 0)
            )
            est_hs_crop = utils.rgba2hs(renders_rgba, bgr_crop)

            est_traj_prev = est_traj
            if False:
                est_traj = renders2traj(renders, device)[0].T.cpu()
            else:
                est_traj = renders2traj_bbox(renders_rgba)
            est_traj = utils.rev_crop_resize_traj(
                est_traj, bbox, (config.resolution_x, config.resolution_y)
            )

            if do_deblatting or do_tbdo:
                if gtp.syn_mode:
                    bbox_temp = utils.bbox_detect_hs(
                        utils.crop_only(
                            gt_hs[:, :, [3, 3, 3], -1], bbox_tight
                        ),
                        utils.crop_only(np.zeros(B.shape), bbox_tight),
                    )
                    debl_dim = (
                        bbox_temp[2:]
                        - bbox_temp[:2]
                        + np.round(0.2 * radius).astype(bbox_temp.dtype)
                    )
                elif gtp.use_hs:
                    bbox_temp = utils.bbox_detect_hs(
                        utils.crop_only(gt_hs[:, :, :3, -1], bbox_tight),
                        utils.crop_only(B, bbox_tight),
                    )
                    if len(bbox_temp) == 0:
                        bbox_temp = bbox_tight
                    debl_dim = (
                        bbox_temp[2:]
                        - bbox_temp[:2]
                        + np.round(0.2 * radius).astype(bbox_temp.dtype)
                    )
                else:
                    # debl_dim = (radius,radius)
                    debl_dim = (int(radius), int(radius))
                bbox_debl = utils.extend_bbox_uniform(
                    bbox_tight.copy(), 0.5 * radius, I.shape
                )
                if do_tbdo and gtp.w_trajgt:
                    rgba_tbd3d_or, Hso_crop = utils.deblatting_oracle_runner(
                        utils.crop_only(I, bbox_debl),
                        utils.crop_only(B, bbox_debl),
                        debl_dim,
                        gt_traj[[1, 0]] - bbox_debl[:2, None],
                    )
                    Hso = utils.rev_crop_resize(
                        Hso_crop[:, :, None, :][:, :, [-1, -1, -1], :],
                        bbox_debl,
                        np.zeros(I.shape),
                    )

                if do_deblatting:
                    (
                        est_hs_tbd_crop,
                        est_hs_tbd3d_crop,
                        rgba_tbd,
                        rgba_tbd3d,
                        est_traj_tbd,
                        Hs_crop,
                    ) = utils.deblatting_runner(
                        utils.crop_only(I, bbox_debl),
                        utils.crop_only(B, bbox_debl),
                        gtp.nsplits,
                        debl_dim,
                    )
                    est_traj_tbd[0] += bbox_debl[1]
                    est_traj_tbd[1] += bbox_debl[0]
                    if gtp.use_hs:
                        gt_hs_debl_crop = utils.crop_only(gt_hs, bbox_debl)[
                            :, :, :3
                        ]
                        est_hs_tbd_crop, do_flip_debl = utils.sync_directions(
                            est_hs_tbd_crop, gt_hs_debl_crop
                        )
                        (
                            est_hs_tbd3d_crop,
                            do_flip_debl,
                        ) = utils.sync_directions(
                            est_hs_tbd3d_crop, gt_hs_debl_crop
                        )
                        if do_flip_debl:
                            rgba_tbd = rgba_tbd[:, :, :, ::-1]
                            rgba_tbd3d = rgba_tbd3d[:, :, :, ::-1]
                    est_hs_tbd = utils.rev_crop_resize(
                        est_hs_tbd_crop, bbox_debl, I
                    )
                    est_hs_tbd3d = utils.rev_crop_resize(
                        est_hs_tbd3d_crop, bbox_debl, I
                    )
                    Hs = utils.rev_crop_resize(
                        Hs_crop[:, :, None, :][:, :, [-1, -1, -1], :],
                        bbox_debl,
                        np.zeros(I.shape),
                    )

            if do_sota18:
                # est_hs_sota18 = run_sota18(I)
                est_hs_sota18_crop = run_sota18(im_crop)
                est_hs_sota18 = utils.rev_crop_resize(
                    est_hs_sota18_crop, bbox, I
                )

            if do_deblurgan:
                # est_hs_deblurgan = run_deblurgan(I)
                est_hs_deblurgan_crop = run_deblurgan(im_crop)
                est_hs_deblurgan_hr = utils.rev_crop_resize(
                    est_hs_deblurgan_crop[..., None], bbox, I
                )[..., 0]

            if gtp.use_hs:
                gt_hs_crop = utils.crop_resize(
                    gt_hs[:, :, :3],
                    bbox,
                    (config.resolution_x, config.resolution_y),
                )
                est_hs_crop, do_flip = utils.sync_directions(
                    est_hs_crop, gt_hs_crop
                )
            else:
                est_hs_crop, est_traj, do_flip = utils.sync_directions_smooth(
                    est_hs_crop, est_traj, est_traj_prev, radius
                )
            if do_flip:
                renders_rgba = renders_rgba[:, :, :, ::-1]

            est_hs = utils.rev_crop_resize(est_hs_crop, bbox, I)
            est_hs[est_hs < 0] = 0

            seq_score_tracker.report(gtp.seqname, kk)
            if do_deblatting:  # and gtp.use_hs
                seq_score_tracker_tbd.next_appearance(
                    kk,
                    utils.crop_only(gt_hs[:, :, :3], bbox_tight),
                    utils.crop_only(est_hs_tbd, bbox_tight),
                    utils.crop_only(I, bbox_tight),
                    utils.crop_only(B, bbox_tight),
                )
                seq_score_tracker_tbd3d.next_appearance(
                    kk,
                    utils.crop_only(gt_hs[:, :, :3], bbox_tight),
                    utils.crop_only(est_hs_tbd3d, bbox_tight),
                    utils.crop_only(I, bbox_tight),
                    utils.crop_only(B, bbox_tight),
                )
                seq_score_tracker_tbd.report(gtp.seqname, kk)
                seq_score_tracker_tbd3d.report(gtp.seqname, kk)

            # pdb.set_trace()
            renders_rgba_full = utils.rev_crop_resize(
                renders_rgba, bbox, np.zeros(I.shape[:2] + (4,))
            )
            # inds = [0,4,gtp.nsplits-1]
            inds0 = np.array([0, 4])
            offsets = np.array([0, 3])
            if results_mode:
                if n_occ == 2:
                    inds0 = np.array([0])
                    offsets = np.array([0, 7])
                else:
                    inds0 = np.array([0])
                    offsets = np.array(range(gtp.nsplits))

            for tempi in offsets:
                inds = inds0 + tempi
                renders_a_joint = renders_rgba_full[:, :, :, inds].sum(3)
                est_hs_joint = B
                renders_rgba_joint = (0.3 ** (1 / gamma_c)) * np.ones(B.shape)
                if gtp.syn_mode:
                    gt_hs_sum = np.ones(B.shape)
                    gt_hs_a = gt_hs[:, :, 3:, inds].sum(3)
                    renders_rgba_joint = np.ones(B.shape)
                elif gtp.use_hs:
                    gt_hs_sum = gt_hs[:, :, :3, inds[0]]

                if do_deblatting:
                    renders_a_joint_tbd = np.zeros(B.shape)
                    renders_a_joint_tbd3d = renders_a_joint_tbd
                    est_hs_joint_tbd = est_hs_joint
                    renders_rgba_joint_tbd = renders_rgba_joint
                    est_hs_joint_tbd3d = est_hs_joint
                    renders_rgba_joint_tbd3d = renders_rgba_joint
                if do_tbdo:
                    renders_a_joint_tbd3do = np.zeros(B.shape)
                    est_hs_joint_tbd3do = est_hs_joint
                    renders_rgba_joint_tbd3do = renders_rgba_joint

                for ind0 in inds:
                    est_hs_joint = utils.rgba2hs(
                        renders_rgba_full[..., ind0 : (ind0 + 1)], est_hs_joint
                    )[..., 0]
                    renders_rgba_joint = utils.rgba2hs(
                        renders_rgba_full[..., ind0 : (ind0 + 1)],
                        renders_rgba_joint,
                    )[..., 0]
                    mask = binary_dilation(
                        renders_rgba_full[:, :, -1, ind0] > 0.001,
                        iterations=10,
                    )[:, :, None][:, :, [0, 0, 0]]
                    if gtp.syn_mode:
                        gt_hs_sum = utils.rgba2hs(
                            gt_hs[..., ind0 : (ind0 + 1)], gt_hs_sum
                        )[..., 0]
                    elif gtp.use_hs:
                        if inds[0] != ind0:
                            gt_hs_sum[mask] = gt_hs[:, :, :3, ind0][mask]

                    if do_deblatting:
                        Hsc = Hs[:, :, 0, ind0] / np.sum(Hs[:, :, 0, ind0])
                        renders_a_joint_tbd = utils.fmo_model(
                            renders_a_joint_tbd,
                            Hsc,
                            rgba_tbd[:, :, [3, 3, 3], ind0],
                            rgba_tbd[:, :, 3, ind0],
                        )
                        est_hs_joint_tbd = utils.fmo_model(
                            est_hs_joint_tbd,
                            Hsc,
                            rgba_tbd[:, :, :3, ind0],
                            rgba_tbd[:, :, 3, ind0],
                        )
                        renders_rgba_joint_tbd = utils.fmo_model(
                            renders_rgba_joint_tbd,
                            Hsc,
                            rgba_tbd[:, :, :3, ind0],
                            rgba_tbd[:, :, 3, ind0],
                        )

                        renders_a_joint_tbd3d = utils.fmo_model(
                            renders_a_joint_tbd3d,
                            Hsc,
                            rgba_tbd3d[:, :, [3, 3, 3], ind0],
                            rgba_tbd3d[:, :, 3, ind0],
                        )
                        est_hs_joint_tbd3d = utils.fmo_model(
                            est_hs_joint_tbd3d,
                            Hsc,
                            rgba_tbd3d[:, :, :3, ind0],
                            rgba_tbd3d[:, :, 3, ind0],
                        )
                        renders_rgba_joint_tbd3d = utils.fmo_model(
                            renders_rgba_joint_tbd3d,
                            Hsc,
                            rgba_tbd3d[:, :, :3, ind0],
                            rgba_tbd3d[:, :, 3, ind0],
                        )
                    if do_tbdo:
                        Hsco = Hso[:, :, 0, ind0] / np.sum(Hso[:, :, 0, ind0])
                        renders_a_joint_tbd3do = utils.fmo_model(
                            renders_a_joint_tbd3do,
                            Hsco,
                            rgba_tbd3d_or[:, :, [3, 3, 3], ind0],
                            rgba_tbd3d_or[:, :, 3, ind0],
                        )
                        est_hs_joint_tbd3do = utils.fmo_model(
                            est_hs_joint_tbd3do,
                            Hsco,
                            rgba_tbd3d_or[:, :, :3, ind0],
                            rgba_tbd3d_or[:, :, 3, ind0],
                        )
                        renders_rgba_joint_tbd3do = utils.fmo_model(
                            renders_rgba_joint_tbd3do,
                            Hsco,
                            rgba_tbd3d_or[:, :, :3, ind0],
                            rgba_tbd3d_or[:, :, 3, ind0],
                        )

                est_hs_joint[est_hs_joint < 0] = 0
                renders_rgba_joint[renders_rgba_joint < 0] = 0
                utils.imwrite(
                    utils.crop_only(
                        1 - renders_a_joint[..., [-1, -1, -1]], bbox_tight
                    ),
                    log_folder
                    + gtp.seqname
                    + "_{:04d}_estm{}.png".format(kk, tempi),
                )
                utils.imwrite(
                    utils.crop_only(renders_rgba_joint, bbox_tight) ** gamma_c,
                    log_folder
                    + gtp.seqname
                    + "_{:04d}_estfm{}.png".format(kk, tempi),
                )
                utils.imwrite(
                    utils.crop_only(est_hs_joint, bbox_tight) ** gamma_c,
                    log_folder
                    + gtp.seqname
                    + "_{:04d}_est{}.png".format(kk, tempi),
                )
                if do_deblatting:
                    renders_a_joint_tbd[renders_a_joint_tbd < 0] = 0
                    renders_a_joint_tbd3d[renders_a_joint_tbd3d < 0] = 0
                    est_hs_joint_tbd[est_hs_joint_tbd < 0] = 0
                    renders_rgba_joint_tbd[renders_rgba_joint_tbd < 0] = 0
                    est_hs_joint_tbd3d[est_hs_joint_tbd3d < 0] = 0
                    renders_rgba_joint_tbd3d[renders_rgba_joint_tbd3d < 0] = 0
                    utils.imwrite(
                        utils.crop_only(
                            1 - renders_a_joint_tbd[..., [-1, -1, -1]],
                            bbox_tight,
                        ),
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbdm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(
                            1 - renders_a_joint_tbd3d[..., [-1, -1, -1]],
                            bbox_tight,
                        ),
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3dm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(renders_rgba_joint_tbd, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbdfm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(renders_rgba_joint_tbd3d, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3dfm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(est_hs_joint_tbd, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(est_hs_joint_tbd3d, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3d{}.png".format(kk, tempi),
                    )

                if gtp.syn_mode:
                    utils.imwrite(
                        utils.crop_only(
                            1 - gt_hs_a[..., [-1, -1, -1]], bbox_tight
                        )
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_hsm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(gt_hs_sum, bbox_tight) ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_hs{}.png".format(kk, tempi),
                    )
                elif gtp.use_hs:
                    gt_hs_a = (
                        np.sum(np.abs(gt_hs - B[..., None]), 2)[
                            :, :, None, :
                        ].astype(float)
                        * 8
                    )
                    utils.imwrite(
                        utils.crop_only(
                            1 - gt_hs_a[..., [-1, -1, -1], inds[0]], bbox_tight
                        )
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_hsm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(gt_hs_sum, bbox_tight) ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_hs{}.png".format(kk, tempi),
                    )

                if do_tbdo:
                    renders_a_joint_tbd3do[renders_a_joint_tbd3do < 0] = 0
                    est_hs_joint_tbd3do[est_hs_joint_tbd3do < 0] = 0
                    renders_rgba_joint_tbd3do[
                        renders_rgba_joint_tbd3do < 0
                    ] = 0
                    utils.imwrite(
                        utils.crop_only(
                            1 - renders_a_joint_tbd3do[..., [-1, -1, -1]],
                            bbox_tight,
                        ),
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3dom{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(renders_rgba_joint_tbd3do, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3dofm{}.png".format(kk, tempi),
                    )
                    utils.imwrite(
                        utils.crop_only(est_hs_joint_tbd3do, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_tbd3do{}.png".format(kk, tempi),
                    )

                if do_sota18:
                    utils.imwrite(
                        utils.crop_only(est_hs_sota18[..., 0], bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_sota18_{}.png".format(kk, 0),
                    )
                    utils.imwrite(
                        utils.crop_only(est_hs_sota18[..., -1], bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_sota18_{}.png".format(kk, 7),
                    )

                if do_deblurgan:
                    # imwrite(crop_only(est_hs_deblurgan,bbox_tight)**gamma_c,log_folder+gtp.seqname+'_{:04d}_deblurgan.png'.format(kk))
                    utils.imwrite(
                        utils.crop_only(est_hs_deblurgan_hr, bbox_tight)
                        ** gamma_c,
                        log_folder
                        + gtp.seqname
                        + "_{:04d}_deblurgan_hr.png".format(kk),
                    )

            I[I < 0] = 0
            B[B < 0] = 0
            utils.imwrite(
                utils.crop_only(I, bbox_tight) ** gamma_c,
                log_folder + gtp.seqname + "_{:04d}_im.png".format(kk),
            )
            utils.imwrite(
                utils.crop_only(B, bbox_tight) ** gamma_c,
                log_folder + gtp.seqname + "_{:04d}_bgr.png".format(kk),
            )
